...
if [[ -z "${SIF}" ]]; then
    ...
    for inputfile in ./input_seq/*.txt; do
        ...
        if peppred_is_local; then
            echo "Running AFFT locally for ${targname}"
            PEPPRED_SCHEDULER=local bash "${ROOT}/AFFT-HLA3DB/predict_structure.sh" "$targname" "$params"
            echo "Finished local AFFT for ${targname}"
        ...
        jid=$(sbatch --parsable \
            --partition="${PEPPRED_GPU_PARTITION:-{{PARTITION_GPU}}}" \
            --gres="${PEPPRED_GPU_GRES:-gpu:1}" \
            --mem="${PEPPRED_AFFT_MEM:-64G}" \
            --time="${PEPPRED_AFFT_TIME:-01:00:00}" \
            --output="$olog" \
            --error="$elog" \
    ...
    if peppred_is_local; then
        if [[ ${local_count} -gt 0 ]]; then
            echo "Running Protpardelle locally after ${local_count} AFFT target(s)."
            cd "${ROOT}/protpardelle"
            PEPPRED_SCHEDULER=local bash "${ROOT}/protpardelle/run.sh"
        else
            echo "No AFFT inputs found, skipping Protpardelle."
        fi
        exit 0
    fi
    ...
    if [ ${#job_ids[@]} -gt 0 ]; then
        ...
        final_jid=$(sbatch --parsable \
            ...
            --partition="${PEPPRED_CPU_PARTITION:-{{PARTITION_SHORT}}}" \
...
# Initialize inputs via container. This stage is CPU-only; GPU jobs are submitted below.
singularity exec \
    --cleanenv \
    --bind "${ROOT}:${ROOT}" \
