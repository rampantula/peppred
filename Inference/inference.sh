...
if [[ -z "${SIF}" ]]; then
    ...
    mkdir -p stored logs "${LOC}/Inference/stored"
    ...
    if peppred_is_local; then
        PEPPRED_SCHEDULER=local bash list.sh
        python dihedrals.py
...
# Helper: run a Python script inside the container with the given env
run_compare() {
...
LOG_DIR="${LOC}/Inference/logs"
mkdir -p "${LOC}/Inference/stored"
mkdir -p "${LOC}/Inference/out"
