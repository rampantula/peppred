...
if [[ -z "${SIF}" ]]; then
    ...
    mkdir -p "${LOC}/AFFT-HLA3DB/MHC_pdbs" "${LOC}/AFFT-HLA3DB/scoring"
    ...
    PARTITION="${PARTITION:-{{PARTITION_GPU}}}"
    GPUS="${GPUS:-1}"
    CPUS="${CPUS:-8}"
    MEM="${MEM:-64G}"
    TIME="${TIME:-08:00:00}"
    ...
    FOLDSEEK_BIN="${FOLDSEEK_BIN:-${ENV}/envs/protpardelle/bin/foldseek}"
...
export FOLDSEEK_BIN PROJECT_NAME PROTPARDELLE_CONFIG_NAME PROTPARDELLE_OUTPUT_DIR PROTPARDELLE_SEED
