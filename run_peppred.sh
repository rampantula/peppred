...
Examples:
  ./run_peppred.sh --env peppred-public.env --input input.csv --run-id review_001
  ./run_peppred.sh --input input.csv --sif /path/to/peppred.sif --netmhcpan /path/to/netmhcpan
USAGE
}
...
if [[ -n "${PEPPRED_SCHEDULER:-}" ]]; then
    PEPPRED_SCHEDULER="$(printf '%s' "$PEPPRED_SCHEDULER" | tr '[:upper:]' '[:lower:]')"
    [[ "$PEPPRED_SCHEDULER" == "slurm" ]] || die "this minimal checkout is Slurm-only; use the unified checkout for PEPPRED_SCHEDULER=${PEPPRED_SCHEDULER}"
    export PEPPRED_SCHEDULER
fi
